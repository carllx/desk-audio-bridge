"""Subprocess-level singleton regression test verifying fail-closed exit code 2."""
...
print("=== Subprocess Singleton Regression Test ===")

# Step 1: Start real controller owner A in foreground/background via run
print("Step 1: Launching Owner A...")
proc_a = subprocess.Popen(
    [python_exe, "-m", "windows.cli", "run"],
...
time.sleep(1.0)
assert proc_a.poll() is None, "Owner A must be running!"
print(f"Owner A running actively with PID {proc_a.pid}")

# Step 2: Attempt to launch Owner B
print("Step 2: Launching Owner B (must be rejected)...")
...
p_status = subprocess.run([python_exe, "-m", "windows.cli", "--json", "status"], capture_output=True, text=True, check=True)
...
print("=== SUBPROCESS SINGLETON REGRESSION TEST PASSED! ===")
